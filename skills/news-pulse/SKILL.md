---
name: news-pulse
description: "本 skill 用于用户要求「股价为什么涨/跌」「异动归因」「最近发生了什么」「要不要重审」等股价异动快速归因时（一般触发线单日 ±5%）。用 4 个并行 Agent 快速情报响应，产出事件时间线 + 异动主因 + 是否重审论文。"
---


# 公司新闻脉搏：股价异动快速归因团队

对 用户指定的输入 进行最近新闻侦察与异动归因。**这不是深度投研，是情报快速响应**——目标是 10 分钟内回答："这家公司最近发生了什么？股价异动的真因是什么？要不要重审投资论文？"
...
- **description**:
  ...
  4. **大 V 观点**：可调用 `python3 ${CLAUDE_PLUGIN_ROOT}/tools/xueqiu_scraper.py` 抓段永平等大 V 最近相关发言
     - 段永平 user_id: `1247347556`
     - 命令示例：`python3 ${CLAUDE_PLUGIN_ROOT}/tools/xueqiu_scraper.py --user-id 1247347556 --keywords {公司名},{股票代码} --output /tmp/dyp-{公司名}.md`
