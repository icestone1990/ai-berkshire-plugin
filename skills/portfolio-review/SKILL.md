...
description: "本 skill 用于用户要求「审视组合」「优化持仓」「组合再平衡」「我的持仓怎么样」等投资组合管理时。从研究公司升级到管理组合。"
---


# 组合管理：从"研究公司"到"管理组合"

对 用户指定的输入 执行投资组合审视与优化。
...
对每个持仓使用 `${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py verify-valuation` 校验估值数据。对每只持仓标注信息丰富度（A/B/C级），C级持仓的分析结论标注低置信度。
...
预期回报估算方法（使用 `${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py three-scenario` 计算）：
