...
description: "本 skill 用于用户要求「精读财报」「看腾讯季报」「财报分析」「解读最新财报」等单一 Agent 财报精读时。基于一手资料（年报/季报/电话会）深度解读财报。"
---
...
**数据验证**：使用 `${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py` 对关键数据进行校验：
