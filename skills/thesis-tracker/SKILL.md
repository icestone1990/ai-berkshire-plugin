---
name: thesis-tracker
description: "本 skill 用于用户要求「追踪投资论文」「检查持仓逻辑」「论文复盘」「买入后跟踪」等投资论文持续追踪时。建立并追踪买入逻辑，事实变了就改结论。"
---


# 投资论文追踪：买入后的纪律系统

对 用户指定的输入 执行投资论文追踪检查。
...
使用 `${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py verify-valuation` 校验估值数据。
