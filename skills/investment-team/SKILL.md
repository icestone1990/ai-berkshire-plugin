...
description: "本 skill 用于用户要求「团队研究某公司」「多角度并行分析」「四角色研究」等需要多 Agent 并行深度研究一家公司时。用 Team 工具组建商业模式/财务/行业/风险四角色并行团队，产出综合报告。"
---


# 投研团队：四角色并行分析框架

对 用户指定的输入 进行团队化投资研究分析。使用 Team 工具创建真正的多Agent并行研究团队。
...
- description 包含：
  ...
  7. **金融严谨性验证（必须使用Bash调用工具，禁止心算）**：
     - 市值验算：`python3 ${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py verify-market-cap --price {价格} --shares {股本} --reported {报告市值} --currency {币种}`
     - 估值验算：`python3 ${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py verify-valuation --price {价格} --eps {EPS} --bvps {每股净资产}`
     - 关键数据交叉验证：`python3 ${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py cross-validate --field {字段} --values '{JSON}' --unit {单位}`
     - 三情景估值：`python3 ${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py three-scenario --price {价格} --eps {EPS} --shares {股本亿} --growth {乐观} {中性} {悲观} --pe {乐观PE} {中性PE} {悲观PE}`
...
python3 ${CLAUDE_PLUGIN_ROOT}/tools/report_audit.py extract \
  --report <报告文件路径>

# Step 2 — 对清单每项从可靠信源取数（参见 skills/financial-data.md）

# Step 3 — 输出准出/打回判决
python3 ${CLAUDE_PLUGIN_ROOT}/tools/report_audit.py verdict \
  --results '<填好的JSON>' \
  --report <报告文件名>
```
