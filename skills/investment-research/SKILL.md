...
description: "本 skill 用于用户要求「研究某公司」「分析腾讯」「做投研」「用四大师框架分析」「这家公司值不值得买」等单公司深度投资研究时。基于巴菲特/芒格/段永平/李录四大师综合框架，产出带明确结论（通过/不通过/灰色 + 价格区间）的研究报告。"
---


# 投资研究：巴菲特-芒格-段永平-李录 四大师综合分析框架

对 用户指定的输入 进行系统化投资研究分析。
...
数据收集完成后，**必须调用 `${CLAUDE_PLUGIN_ROOT}/tools/financial_rigor.py` 对关键数据进行程序化验证**，杜绝LLM心算误差。
